...
class SimDynMot():
    ...
    def process(self):
        ...
        self.wrenchThruster.force.x = self.thruster.maxThrust*self.cmd_thrust / 100.0 * np.cos(self.cmd_pos / 180.0 * np.pi)
        self.wrenchThruster.force.y = self.thruster.maxThrust*self.cmd_thrust / 100.0 * np.sin(self.cmd_pos / 180.0 * np.pi)
